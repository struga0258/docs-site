...
Users can also validate JWT using [JSON Web Key](https://auth0.com/docs/secure/tokens/json-web-tokens/json-web-key-sets), that is available via REST endpoint `/gateway/api/v1/auth/keys/public/all` (preferred option) and `/api/v1/gateway/auth/keys/public/all`.
